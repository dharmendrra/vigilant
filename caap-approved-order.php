...
$qry="SELECT tom.*,tpd.BlueBookCode FROM  `tblordersmaster` tom LEFT JOIN tblprogramdirectors tpd ON tom.ProgramDirectorsID = tpd.ProgramDirectorsID WHERE tom.`OrderApproved`='Approved'";
                ...
                <table id="pd-table">
                    ...
                    <?php  while($item=$result_data->fetch_assoc()){  ?>
                        <tr>
                            ...
                            <?php }*/ ?>
                            <td><a title="View Order" href="caap-order.php?oid=<?php echo $item['OrderNumber'];?>&status=apr&user=caap"><i class="icon-view"></i></a></td>
                        </tr>
                    <?php }  ?>
                    </tbody>
                </table>
