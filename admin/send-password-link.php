            ...
            <div class="form-header header-primary" id="menu">
            	...
            	<div class="form-body">
                    <div class="spacer-t40 spacer-b40">
                    	<div class="tagline"><span>::Send Reset Password Link to Program Directors </span></div><!-- .tagline -->
                    </div>
                    ...
                    <div class="frm-row">
                        ...
                        ?>
                        <table id="pd-table">
                            <thead><tr><th>First Name</th><th>Title</th><th>Facility Name</th><th>Email Address</th><th>Phone Number</th><th>Action</th></tr></thead>
                            <?php while($item=$result_data->fetch_assoc()){ 
                                ...
                                $name[]=$item['LastName'];
                                ?>
                                <tr class="id_<?php echo $item['Log-inID'];?>">
                                    <td><?php echo implode(' ',$name);?></td>
                                    <td><?php echo $item['Title'];?></td>
                                    <td><?php echo $item['FacilityName'];?></td>
                                    <td><?php echo $item['EmailAddress'];?></td>
                                    <td><?php echo $item['PhoneNumber'];?></td>
                                    <td title="" onclick="sendPasswordEmail('<?php echo $item['Log-inID'];?>');"><a title="Send Password Link" href="javascript:void(0);"><i class="icon-view send"></i></a></td>
                                    <style>.send{background-position: 0;background-size: 100%;width: 27px;height: 27px;    background-image: url("https://cdn4.iconfinder.com/data/icons/proglyphs-mailing-and-messaging/512/Paper_Plane-32.png");}</style>
                                </tr>
                            <?php }  ?>
                        </table>
                        <?php } ?>
...
<script type="text/javascript">
function sendPasswordEmail(login_id){
    $.post( "../ajax/send-password-email.php", {loginid:login_id})
          .done(function( response ) {
            console.log(response);
            var data = JSON.parse(response);
            console.log(data);
            if(data.RESPONSE=='SENT'){
                alert('A password reset link has been sent to "'+login_id+'"');
                //alert('Your request has been sent successfully.');
                $('.id_'+login_id).slideUp(1500);
            }else{
                alert('Some error');
            }
          });
