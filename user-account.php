...
if(isset($_POST['submit'])){
    ...
    $flag=true;
    $qry="SELECT * FROM `tbluseraccounts` WHERE `tbluseraccounts`.`EmailAddressUser`='".$mysqli->real_escape_string($_POST['EmailAddress'])."' AND `AccountType`='".$mysqli->real_escape_string($_POST['AccountType'])."'";
    $result = $mysqli->query($qry) or die($mysqli->error.' '.$qry);
    if($result->num_rows<=0){
        //insert query- to feed posted tblprogramdirectors
        $insertQuery = "INSERT INTO `tbluseraccounts` (`FirstNameUser`,`LastNameUser`, `EmailAddressUser`,`PasswordUser` ,`ActiveUser`,`AccountType`) VALUES 
        ('".$mysqli->real_escape_string($_POST['FirstName'])."',
         ...
         '".$mysqli->real_escape_string($_POST['AccountType'])."');";
...
//fetch data for the user account
$qry="SELECT * FROM  `tbluseraccounts` WHERE `AccountType`!='Admin'";
            ...
            <form method="post" action="" id="form-ui" enctype="multipart/form-data" onsubmit="return validate_pd();">
            	<div class="form-body">
                    ...
                    <div class="spacer-t40 spacer-b40">
                    	<div class="tagline"><span class="large-text"> Add User Account </span></div><!-- .tagline -->
                    </div>
                    <br />
                            ...
                            <div class="section">
                                ...
                                <label class="field select">
                                    <select required="true" id="AccountType" name="AccountType">
                                        <option value="CustServiceUser">Customer Service</option>
                                        <option value="ComponentUser">Healogics Administrator</option>
                                    </select>
                                    <i class="arrow"></i>
                                </label>  
                            </div>
                    ...
                    <div class="frm-row">
                        <div class="colm colm6">    
                            <div class="section">
                                ...
                                <label class="field">
                                    <input required="true" type="text" name="Password" id="Password" class="gui-input" placeholder="Password">
                                </label>
                            </div><!-- end section -->
                        ...
                        <div class="colm colm6">    
                            <div class="section heading">
                                <label for="SiteName" class="link-password field" style="cursor:pointer;" id="confirm">Use Password</label>
                                <label class="field">
                                    <input type="text" name="random" id="random" class="link-password gui-input">
                	...
                	<div class="tagline"><span><input type="submit" name="submit" id="submit" class="button btn-primary" value="Add User"></span></div><!-- .tagline -->
                </div>
                                                                                                                 
                </div><!-- end .form-body section -->
                
                <div class="spacer-t40 spacer-b40">
                	<div class="tagline"><span class="large-text"> User Account(s) </span></div><!-- .tagline -->
                </div>
                ...
                <div class="frm-row">
                    ...
                    ?>
                    <table id="pd-table">
                        <thead><tr><th>First Name</th><th>Last Name</th><th>Email</th><th>Password</th><th>User Type</th><th>Controls</th></tr></thead>
                        <?php while($account=$accounts->fetch_assoc()){ ?>
                            <tr>
                                <td><?php echo $account['FirstNameUser'];?></td>
                                <td><?php echo $account['LastNameUser'];?></td>
                                <td><?php echo $account['EmailAddressUser'];?></td>
                                <td style="font-weight: bold;color: green;"><span style="background-color: gray;color: #FFF;"><?php echo $account['PasswordUser'];?></span></td>
                                <td><?php echo $account['AccountType'];?></td>
                                <td><a title="Edit" href="edit-user-account.php?uid=<?php echo $account['UserAccountID'];?>&a_t=<?php echo $account['AccountType'];?>"><i class="icon-edit"></i></a></td>
                            </tr>
                        <?php }  ?>
                    </table>
                    <?php } ?>
