...
if(isset($_POST['submitOrder'])){
    ...
    $add='';$add1='';$add2='';
    $add1[]=$_SESSION['USER_INFO']['CompanyAddress1'];
    $add1[]=$_SESSION['USER_INFO']['CompanyAddress2'];
    $add2[]=$_SESSION['USER_INFO']['City'];
    $add2[]=$_SESSION['USER_INFO']['State'];
    $add3[]=$_SESSION['USER_INFO']['Zip'];
    $add3[]=$_SESSION['USER_INFO']['Country'];
    $add = implode(', ',$add1).'<br>'.implode(', ',$add2).'<br>'.implode(', ',$add3);
    
    $DateOrdered = date('Y-m-d h:i:s');
    if($shipType == 'Ground'){
        $PromisedDate = date('Y-m-d h:i:s',strtotime('+1 day',strtotime($DateOrdered)));
    }else{
        $PromisedDate = $DateOrdered;
    }
    $PONumber = PONumber($mysqli);
    //insert query- to feed posted tblitemmaster
    $insertOrdQuery = "INSERT INTO `tblordersmaster` (`OrderNumber`,`ProgramDirectorsID`, `SiteName`,`BlueBookCode`,`CompanyAddress1`,`CompanyAddress2`,`City`, `State`,`Country`,`Zip`, `FirstName`, `LastName`, `EmailAddress`, `ShipType`,`PromisedDate`,`PONumber`,`CustomerID`, `ShipToID`,`OrderPlacedByAddress`,`DateOrdered`,`OrderApproved`) VALUES (
    '".($OrdNum)."', 
    '".$_SESSION['USER_INFO']['ProgramDirectorsID']."',
    '".$_SESSION['USER_INFO']['FacilityName']."',
    '".$_SESSION['USER_INFO']['BlueBookCode']."',
    '".$_SESSION['USER_INFO']['CompanyAddress1']."', 
    '".$_SESSION['USER_INFO']['CompanyAddress2']."', 
    '".$_SESSION['USER_INFO']['City']."', 
    '".$_SESSION['USER_INFO']['State']."', 
    '".$_SESSION['USER_INFO']['Country']."', 
    '".$_SESSION['USER_INFO']['Zip']."', 
    '".$_SESSION['USER_INFO']['FirstName']."', 
    '".$_SESSION['USER_INFO']['LastName']."', 
    '".$_SESSION['USER_INFO']['EmailAddress']."', 
    '".$mysqli->real_escape_string($shipType)."', 
    '".$PromisedDate."', '".$PONumber."',
    '".$_SESSION['USER_INFO']['CustomerID']."', 
    '".$_SESSION['USER_INFO']['ShipToID']."',
    '".$mysqli->real_escape_string($_SESSION['USER_INFO']['EmailAddress'])."',
    '".date('Y-m-d h:i:s')."',
    'Pending')";
    	...
    	<div class="smart-forms smart-container wrap-2">
            ...
            <form method="post" action="" id="lineItemForm" enctype="multipart/form-data" onsubmit="//return validate_lineItem();">
            	<div class="form-body">
                    ...
                    <div class="colm colm6">
                        <div class="section">
                            ...
                            <label for="ProgramDirectorID" class="field select">
                                   <select name="ProgramDirectorID" id="ProgramDirectorID" onchange="if(this.value){window.location.href='program-director-dashboard.php?pd='+this.value+'&q='+$.trim($('#pd-selector').val());}">
                                   <option value="">Choose Site</option>
                                        <?php
                                        //SELECT COUNT(*) AS `Rows`,GROUP_CONCAT(`tblprogramdirectors`.ProgramDirectorsID separator ','),GROUP_CONCAT(`tblprogramdirectors`.Log-inID separator ','),`tblprogramdirectors`.BlueBookCode FROM `tblprogramdirectors` LEFT JOIN `tblhealogicssitemaster` ON `tblprogramdirectors`.BlueBookCode = `tblhealogicssitemaster`.BlueBookCode  GROUP BY `BlueBookCode` ORDER BY `Rows` DESC
                                        //list all Program Directors
                                        $qry="SELECT tpd.ProgramDirectorsID,tpd.`Log-inID`,tpd.EmailAddress ,tpd.BlueBookCode,tpd.FirstName,tpd.LastName,thsm.CustomerID,thsm.ShipToID,thsm.CompanyAddress1,thsm.CompanyAddress2,thsm.FacilityName,thsm.City,thsm.State,thsm.Zip,thsm.Country  FROM `tblprogramdirectors` tpd , `tblhealogicssitemaster` thsm WHERE `thsm`.FacilityStatus = 'A' AND `tpd`.BlueBookCode = `thsm`.BlueBookCode AND tpd.`Log-inID` = '".$_SESSION['USER_INFO']['Log-inID']."' ORDER BY tpd.BlueBookCode";
                    ...
                    <div class="frm-row" <?php if($PD_selected==false){echo "style='display:none'";}?>>
                        <!-- program director info -->
                        <div class="colm colm6">                   
                            <label class="field large-text fine-black"> <div class="align-div">Facility</div><?php echo $_SESSION['USER_INFO']['FacilityName'];?> </label>
                            <label class="field large-text fine-black"> <div class="align-div">Name</div><?php echo $_SESSION['USER_INFO']['FirstName'].' '.$_SESSION['USER_INFO']['LastName'];?></label>
                            <label class="field large-text fine-black"> <div class="align-div">Email</div><?php echo $_SESSION['USER_INFO']['EmailAddress'];?></label>
                        </div>
                        ...
                        <div class="colm colm6">
                                ...
                                $address[]=$_SESSION['USER_INFO']['CompanyAddress1'];
                                if($_SESSION['USER_INFO']['CompanyAddress2']):
                                $address[]=$_SESSION['USER_INFO']['CompanyAddress2'];
                                endif;
                                $address1[]=$_SESSION['USER_INFO']['City'];
                                $address1[]=$_SESSION['USER_INFO']['State'];
                                $address2[]=$_SESSION['USER_INFO']['Zip'];
                                $address2[]=$_SESSION['USER_INFO']['Country'];
                                echo implode(', ',$address).'<br>';
                                echo implode(', ',$address1).'<br>';
                                echo implode(', ',$address2);?>  
                                </label><!--
                                <label class="field large-text fine-black">
                                Blue Book Code: <?php print $_SESSION['USER_INFO']['ProgramDirectorsID'].' '.$_SESSION['USER_INFO']['BlueBookCode']; ?>
                                </label>-->
                        </div>
                    </div>                    
                    ...
                    <div class="frm-row">
                        ...
                        <div class="colm colm6">
                            ...
                            <div class="section">
                                ...
                                <?php
                                //fetch soil types from database
                                $qry="SELECT * FROM `tblitemmaster` ORDER BY ItemID";
                            ...
                            <div class="colm colm12">
                                <div class="section">
                                    ...
                                    <label for="ItemDescription" class="field prepend-icon">
                                    	<textarea class="gui-textarea" readonly="" id="ItemDescription" name="ItemDescription" placeholder="Add Description"></textarea>
                                        <label for="ItemDescription" class="field-icon"><i class="fa fa-comments"></i></label>
                                        <span class="input-hint"> 
                                        	<strong>Note:</strong> Some items need management approval and also have maximum quantity on order... 
                                        </span>   
                                    </label>
                                </div>
                            </div>
...
$(function() {
    $('#lineItemForm').submit(function() {
        ...
        */
        ...
        if(parseInt($.trim($('#MaxUnit').val()))<=0){
	        alert('Quantity of 0 not allowed on line item. Enter correct quantity and add line item. ');
	        return false;
	}
